...
const rememberMeCheckboxSelector = '#email-signin input[name="remember"]'
...
const publish = async ({ credentials, script, description = "", interactive = false }) => {
	...
	await page.type('#email-signin input[name="username"]', credentials.username)
	await page.type('#email-signin input[name="password"]', credentials.password)
	...
	await Promise.all([
		...
		page.click('#email-signinCCccccccc button[type="submit"]')
	])
