...
const publish = async ({ credentials, script, description = "", interactive = false }) => {
	...
	await page.type('#email-signin__form input[name="username"]', credentials.username)
	await page.type('#email-signin__form input[name="password"]', credentials.password)
	...
	await Promise.all([
		...
		page.click('#email-signin__form button[type="submit"]')
	])
